...
import time
try:
    fechadecreacion = int(os.path.getmtime('registros.csv'))
    print(fechadecreacion)
    fechaactual = int(time.time())
    diferencia = fechaactual - fechadecreacion
    print(diferencia)
    if diferencia > 3600:
        #Descarga de archivo
...
#plot 3:#####################################################
cursor.execute('''
    SELECT COUNT(navegador) FROM registros
...
for i in datos:
    numeros2.append(i[0])   
conn.commit()
etiquetas = ["Mac","Windows"]
plot.subplot(3, 3, 3)
print(numeros2)
print(etiquetas)
plot.pie(numeros2,labels = etiquetas)
plot.title('Windows versus Mac')
#plot 3:#####################################################
