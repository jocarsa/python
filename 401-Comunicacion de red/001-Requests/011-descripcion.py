...
def compruebaSEO():
    ...
    try:
        ...
        if len(textos) == 0:
            campodetexto.image_create(tk.END, image = ko)
            campodetexto.insert(tk.END, '\n')
            campodetexto.insert(tk.END, 'Comprobación de descripción\n')
            campodetexto.insert(tk.END, 'No se encuentra etiqueta de descripción\n')
            
        else:
            campodetexto.image_create(tk.END, image = ok)
            campodetexto.insert(tk.END, '\n')
            campodetexto.insert(tk.END, 'Comprobación de descripción\n')
            campodetexto.insert(tk.END, 'Se ha encontrado una descripción:\n')
            campodetexto.insert(tk.END, str(textos[0])+"\n")
            campodetexto.insert(tk.END, "Longitud de la descripción: "+str(len(str(textos[0])))+"\n")         
